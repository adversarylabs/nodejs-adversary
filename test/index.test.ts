...
const ruleCases = [{"key":"dynamic-eval","id":"nodejs.dynamic-eval"},{"key":"shell-exec","id":"nodejs.shell-exec"},{"key":"tls-disabled","id":"nodejs.tls-disabled"}];
...
test("output ordering and protocol envelope are deterministic", async () => {
  ...
  assert.equal(envelope.result.adversary.name, "nodejs");
});
